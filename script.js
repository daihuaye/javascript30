...
const sorted = inventors.sort((prev, next) => { 
  return prev.year - next.year > 0 ? 1 : -1;
});
...
const numYears = inventors.reduce((sum, inventor) => {
...
const sortedYears = inventors.sort((prev, next) => {
  const pYears = prev.passed - prev.year;
  const nYears = next.passed - next.year;
  
  return pYears - nYears > 0 ? -1 : 1;
});
...
// const cities = Array.from(document.querySelectorAll('.mw-category-group a'));
// const cityNames = cities.map(city => city.text);
// const deCities = cityNames.filter(name => name.includes('de'));
...
const sortedPeople = people.sort((prev, next) => {
  ...
  return pLast.localeCompare(nLast);
});
